...
public class AmphipodNavigator {    
    private static final int NUM_HALLWAYS_POSITIONS = 11;
    private static final int NUM_ROOMS = 4;
    private static final int NUM_POSITIONS = NUM_HALLWAYS_POSITIONS + 2 * NUM_ROOMS;
    ...
    private static boolean isLowerRoom(int room) {
        return room >= 15 && room <= 18;
    }
    ...
    private static int getExitPosition(int room) {
        if (room == 11 || room == 15) {
            ...
            return 4;
        } else if (room == 13 || room == 17) {
            return 6;
        } else if (room == 14 || room == 18) {
            return 8;
        }
    ...
    private static int getLowerRoomPosition(char name) {
        if (name == 'A') {
            return 15;
        } else if (name == 'B') {
            return 16;
    ...
    private static int getUpperRoomPosition(char name) {
        if (name == 'A') {
            return 11;
        } else if (name == 'B') {
            return 12;
    ...
    public static Map<AmphipodState, Integer> getOpenChildStates(AmphipodState parent, Set<AmphipodState> closed) {
        ...
        for (var entry : parent.agents.entrySet()) {
            ...
            int targetLowerRoom = getLowerRoomPosition(agent.name);
            int targetUpperRoom = getUpperRoomPosition(agent.name);
            ...
            if (agentStartPosition == targetLowerRoom) {
                continue;
            }
            // If high in the right room, don't allow a move unless blocking someone that must move.
            if (agentStartPosition == targetUpperRoom && parent.agents.containsKey(targetLowerRoom)) {
                Agent lowerAgent = parent.agents.get(targetLowerRoom);
            ...
            if (isHallway(agentStartPosition)) {
                ...
                int targetRoom;
                if (!parent.agents.containsKey(targetLowerRoom)) {
                    // Move into lower room
                    targetRoom = targetLowerRoom;
                    pathLength += 2;
                } else if (parent.agents.get(targetLowerRoom).name == agent.name) {
                    // Move into upper room
                    targetRoom = targetUpperRoom;
                    pathLength += 1;
                } else {
                    continue;
                }
                ...
                int pathLengthToHallway;
                // Move out to the hallway if possible
                if (isLowerRoom(agentStartPosition)) {
                    if (parent.agents.containsKey(agentStartPosition - 4)) {
                        // Blocked from moving to hallway
                        continue;
                    }
                    pathLengthToHallway = 2;
                } else {
                    pathLengthToHallway = 1;
                }
    ...
    public static void main(String[] args) {
        ...
        start.addAgent(new Agent("A1", 'A'), 15);
