...
public class AimNavigator {
    ...
    private static class Position {
        ...
        public Position(long x, long y, long aim) {
            this.x = x;
            this.y = y;
            this.aim = aim;
        }
    ...
    private static Position applyInstruction(Position start, Instruction instruction) {
        if (instruction.direction == Direction.UP) {
            return new Position(start.x, start.y, start.aim - instruction.magnitude);
        } else if (instruction.direction == Direction.DOWN) {
            return new Position(start.x, start.y, start.aim + instruction.magnitude);
        } else if (instruction.direction == Direction.FORWARD) {
            return new Position(start.x + instruction.magnitude, start.y + (start.aim * instruction.magnitude), start.aim);
        } else {
            return null;
        }
    }
